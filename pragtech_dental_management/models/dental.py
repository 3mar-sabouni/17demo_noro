...
class MedicalPhysician(models.Model):
    ...
    def name_search(self, name, args=None, operator='ilike', limit=100):
        if self.env.context.get('start_date') and self.env.context.get('end_date'):
            ...
            return super(MedicalPhysician, self).name_search(name, args=[('id', 'in', doctor_list)], operator='ilike', limit=limit)

        return super(MedicalPhysician, self).name_search(name, args=None, operator='ilike', limit=limit)
...
class MedicalPatient(models.Model):
        ...
        try:
            if treatment_ids:
                for treatment in treatment_ids:
                    ...
                    teeth_codes = treatment.teeth_code_rel.mapped('name')
                    data.append({
                        'create_date': treatment.create_date,
                        'description': treatment.description.name if treatment.description else '',
                        'teeth_id': treatment.teeth_id.id,
                        'teeth_code_rel': teeth_codes,  # Return the list of tooth names
                        'state': treatment.state,
                        'dentist': treatment.dentist.name if treatment.dentist else '',
    ...
    #                 data.append({
    #                     'create_date':treatment.create_date,
    #                     'description':treatment.description.name,
    #                     'teeth_id':treatment.teeth_id.id,
    #                     'state':treatment.state,
    ...
    def get_material_history(self, **kw):
        material_ids = self.env['material.record'].search([('patient', "=", kw.get('patient_id'))])
        try:
            if material_ids:
                data = []
