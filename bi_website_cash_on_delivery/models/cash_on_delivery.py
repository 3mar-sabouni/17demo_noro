    ...
    def min_max_amt_calculation(self):
        ...
        for p in cod_product_ids:
            if p not in self.cod_config.excl_product:
                if p.list_price <= self.cod_config.min_amt and p.list_price >= self.cod_config.max_amt:
...
class CODPaymentTransaction(models.Model):
    _name = 'cod.config'
    _description = "Cod Config"

    name = fields.Char('Name', required=True)
    min_amt = fields.Float('Minimum Order Amount',default=1, required=True)
    ...
    @api.onchange('min_amt','max_amt')
    def min_max_amt_calculation_check(self):
        cash_on_product = self.env['product.template'].sudo().search([])
        for p in cash_on_product:
            if p.list_price >= self.min_amt and p.list_price <= self.max_amt:
                p.write({'cod_available': True})
            else:
                p.write({'cod_available': False})
