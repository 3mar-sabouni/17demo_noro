...
patch(NavBar.prototype, {
    async setup(parent, menuData) {
        ...
        this.showCommandPalette = function () {
            if (self.dialog && self.dialog.isOpen) return;

            self.dialog = new Dialog(self, {
                title: "Command Palette",
                body: self.env.qweb.renderToString("web.CommandPalette"),
                size: "md",
                buttons: [],
            });
            self.dialog.open();
    ...
    _onKeyDown(event) {
        if (!$("body").hasClass("app_drawer_search")) {
            if (!this.command) return;
        
            const allowedKeys = /^[a-zA-Z0-9]$/;
            const focusedInput = document.activeElement;
            if (!$(".appdrawer_section").hasClass("toggle")) {
                if (!focusedInput || !focusedInput.closest(".appdrawer_section")) {
                    return;
                }
            
                const currentType = focusedInput.getAttribute("type");
            ...
            if (allowedKeys.test(event.key) && !event.ctrlKey && !event.altKey && !event.metaKey) {
                event.preventDefault();
                this.inputBuffer = this.inputBuffer ? this.inputBuffer + event.key : event.key;
            ...
            this.typingTimer = setTimeout(() => {
                this._openSearchModal(this.inputBuffer);
            }, this.typingDelay);
        }
    },
    
    _openSearchModal(initialValue = "") {
        if (!this.command) return;
        ...
        if ($(".appdrawer_section").hasClass("toggle")) {
            ...
            const inputValue = this.inputRef?.el?.value?.trim() || initialValue;
            const searchValue = this.compositionStart ? "/" : `/${inputValue.slice(1)}`;
            this.compositionStart = false;
    
            this.command.openMainPalette({ searchValue, FooterComponent: null }, onClose);
        } else {
    ...
    addconfiguratorclass: function () {
        jsonrpc('/get/model/record').then(function (rec) {
            ...
            if (rec.record_dict[0].app_drawer_search) {
                $("body").addClass("app_drawer_search");
                $(".app-drawer-search").removeClass("d-none");
            }
    ...
    showeditmodal: function (ev) {
        $.get('/color/pallet/data/', {}).then(function (data) {
            ...
            $('.top_menu_style').unbind().on('click ', function (e) {
                if ($(this).val() == 'top_menu_horizontal') {
                    $('.top-menu-horizontal-searchbar').removeClass('d-none')
                } else {
                    $('.top-menu-horizontal-searchbar').addClass('d-none')
                }
            });
            ...
            $(".selected_value").on('click', function () {
                ...
                var app_drawer_search = $("input[id='app_drawer_search']").is(':checked')
                ...
                jsonrpc('/color/pallet/', {
                    ...
                    'app_drawer_search': app_drawer_search,
    ...
    addFontManager: function () {
        ...
        $(document).on("click", ".delete-font", function () {
            ...
            jsonrpc('/delete/google/font', { 'id': fontId }).then(function (response) {
    ...
    _onCheckboxChange: function (ev) {
        ...
        jsonrpc("/update_single_font_selection", {
